...
import { DescontoStrategy, PagamentoStrategy, NotificacaoStrategy } from "./strategies/implementations";
...
export class ProcessadorPedido {
    ...
    public finalizarPedido(pedido: Pedido): void {
        ...
        console.log(`Valor Original: R$ ${pedido.valorTotalBruto.toFixed(2)}`);

        const desconto = this.descontoStrategy.calcularDesconto(pedido);
        const valorComDesconto = pedido.valorTotalBruto - desconto;
